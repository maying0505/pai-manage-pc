...
@inject('assetAdd')
@observer
class AssetInquestForm extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            searchArr: [
            ...
            columns:[
                ...
                },
                {
                    ...
                    {record.status === 1 || record.status === 2 ? 
                        <Button size="small" onClick={this.editDetail.bind(this,text,record.status,record.type,record.inquestIdTwo)}>勘验</Button>:
    ...
    loadlists = async (data) => { //请求数据函数
        console.log(data)
        try {
            const result = await HTTP.getbidInvestigate({
                ...data
            });
    ...
    editDetail = async (id,status,type,inquestId) => { //判断是否为主要任务人、跳转详情
        this.setState({
            loading: true
        })
        try {
            ...
            if (result.success) {
                ...
                sessionStorage.setItem('pageName','asset_inquest_detail')
                ...
                let href = '';
                switch(type) {
                    case '0': href = `/project/asset_inquest/asset_detail/house_property/${result.data}/${result.data2 ? result.data2 : null}`;break;
                    case '3': href = `/project/asset_inquest/asset_detail/other/${result.data}/${result.data2 ? result.data2 : null}`;break;
                    case '1': href = `/project/asset_inquest/asset_detail/vehicle/${result.data}/${result.data2 ? result.data2 : null}`;break;
                    case '2': href = `/project/asset_inquest/asset_detail/land/${result.data}/${result.data2 ? result.data2 : null}`;break;
                    default: href = '';return;
                }
                if (status === 1 || status === 2) {
                    sessionStorage.setItem('pageTitle','标的勘验');
                } else {
            ...
            <div className="table-list">
                <Card bordered={false}>
                        <Form onSubmit={this.handleSearch} layout="inline" className="ant-search-form">
                            <FormSet form={this.props.form} searchArr={this.state.searchArr} defaultValues={this.state.searchInfo}/>
                            <Row>
                                <Col xl={12} lg={24}>
...
const AssetInquest = Form.create()(AssetInquestForm)
export default AssetInquest
